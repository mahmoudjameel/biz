...
const HomeScreen = ({ navigation }) => {
  ...
  const [isListView, setIsListView] = useState(false); // افتراضيًا، نعرض list-sharp
  ...
  useEffect(() => {
    // تعيين الحالة كقيمة افتراضية عند التحميل
    handleLayoutToggle(true); // الضغط على الزر تلقائيًا
  }, []);
  ...
  const handleLayoutToggle = (layout) => {
    ...
    setIsListView(layout); // تحديث الحالة
  };
  ...
  const handleLoadTopCategories = () => {
    api.get("categories").then((res) => {
      if (res.ok) {
        // ترتيب مخصص للفئات
        const categoryOrder = ["وظائف", "عقارات", "عربيات"];
  
        // تصفية العناصر لتحتوي فقط على عربيات، عقارات، وظائف
        const filteredCategories = res.data.filter(
          (category) =>
            category.name === "عربيات" ||
            category.name === "عقارات" ||
            category.name === "وظائف"
        );
  
        // فرز العناصر بناءً على الترتيب المخصص
        const sortedCategories = filteredCategories.sort(
          (a, b) => categoryOrder.indexOf(a.name) - categoryOrder.indexOf(b.name)
        );
  
        // تحديد أول 3 عناصر فقط
        const tempTopCatData = sortedCategories.slice(0, 3);
    ...
    <Animated.View>
      ...
      {rtl_support ? (
        ...
        >
          ...
          >
    <TouchableOpacity
        onPress={() => handleLayoutToggle(false)} // اضغط للتبديل إلى List
        style={{
          padding: 4,
          borderRadius: 3,
          borderWidth: 1,
          backgroundColor: isListView ? COLORS.primary : COLORS.white, // عرض List إذا كانت الحالة true
          borderColor: isListView ? COLORS.primary : COLORS.white,
        }}
      >
        <Ionicons
          name="list-sharp"
          size={15}
          color={isListView ? COLORS.white : COLORS.gray} // لون الرمز يتغير بناءً على الحالة
        />
      </TouchableOpacity>
        ...
        >
          ...
          >
     <TouchableOpacity
        onPress={() => handleLayoutToggle(true)} // اضغط للتبديل إلى List
        style={{
          padding: 4,
          borderRadius: 3,
          borderWidth: 1,
